...
import { InMemoryLikeStorage } from './memory-like.storage';

describe('BigtableLikeStorage', () => {
  // MARK: - Properties
  let storage: InMemoryLikeStorage;
  ...
  beforeEach(async () => {
    const app: TestingModule = await Test.createTestingModule({
      ...
      controllers: [],
      providers: [InMemoryLikeStorage],
    }).compile();

    storage = app.get<InMemoryLikeStorage>(InMemoryLikeStorage);
  ...
  it('should return `undefined` if user has not liked object', async () => {
    expect(await storage.getLike(OBJECT_ID, OBJECT_TYPE, USER_1)).toEqual(
      undefined,
    );
  });
